...
The partial autocorrelation $r_{xx}[k,l]$ is the autocorrelation in $(k-l)$ of the modified vector.
...
rxx = xcorr(x) % Calculates the partial autocorrelation, the full vector

L = length(x);			 % We want the autocorelation in [k-l]
rxx(L+k-l)
